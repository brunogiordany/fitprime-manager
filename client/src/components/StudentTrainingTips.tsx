...
const PRE_WORKOUT_TIPS: Tip[] = [
  {
    ...
    icon: Droplets,
    title: "Hidratação",
    description: "Beba 500ml de água nas próximas 2 horas",
    timing: "2h antes",
    color: "text-blue-600",
    bgColor: "bg-blue-50",
  },
...
const POST_WORKOUT_TIPS: Tip[] = [
  {
    ...
    icon: Droplets,
    title: "Reidratação",
    description: "Beba pelo menos 500ml de água após o treino",
    timing: "Imediato",
    color: "text-blue-600",
    bgColor: "bg-blue-50",
  },
...
export default function StudentTrainingTips({ nextSession, studentGoal }: StudentTrainingTipsProps) {
  ...
  const waterGoal = 8; // 8 copos de água por dia
  ...
  useEffect(() => {
    ...
    const storedWater = localStorage.getItem(`waterIntake_${today}`);
    if (storedWater) {
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-sm text-blue-600">
                {waterIntake} de {waterGoal} copos
              </span>
              <span className="text-sm font-medium text-blue-700">
                {Math.round((waterIntake / waterGoal) * 100)}%
              </span>
            </div>
            <Progress value={(waterIntake / waterGoal) * 100} className="h-3 bg-blue-100" />
